...
export function createContentBackup(content: SiteContent, createdAt = new Date()) {
  ...
  const heading = "# Signal Weekly Content Backup";
  const summary = [
    `Created: ${timestamp}`,
    "",
    "> This backup contains the editable Signal Weekly website content, public resources and redirect links.",
    ...
    "",
    "## Recovery note",
    "To restore content, sign in to the Signal Weekly Editor and copy the relevant values back into the matching fields. Do not upload this file publicly because it may contain unpublished draft wording or private destination links.",
  ];
...
export function contentBackupFilename(createdAt = new Date()) {
  return `signal-weekly-content-backup-${createdAt.toISOString().slice(0, 10)}.md`;
}
